"""
IBD (GSE282122) verified re-run — NumPy/parallel version independently
re-implemented from pipeline.py; not a shared code path. Outputs were verified
to match the committed artifacts rather than enforced by a shared module or an
equivalence test. It uses nested LOO-CV + genuine nested permutation logic, but memory-light so the
...
def loo_predict(yv, k=5, track=False):
    ...
    for i in range(n):
        ...
        mu = Xg_tr.mean(0); sd = Xg_tr.std(0, ddof=1); sd[sd==0]=1.0  # ddof=1 follows the pandas convention; independently re-implemented from pipeline.py, not a shared code path (outputs verified against the committed artifacts, not by an equivalence test)
        ...
        f, _ = f_classif(Xtr, yv[tr])          # sklearn f_classif; independently re-implemented from pipeline.py, not a shared code path (outputs verified against the committed artifacts, not by an equivalence test)
