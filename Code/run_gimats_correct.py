...
Consensus genes are derived at runtime from consensus_counts in
ibd_correct_results.pkl: the top five genes by LOO selection frequency
across the 36 TAURUS folds (stable sort on count only, no secondary
tiebreak on gene name).
No target numbers referenced; reports whatever it gets.
"""
import numpy as np, anndata as ad, pickle
from sklearn.ensemble import RandomForestClassifier

with open('ibd_correct_results.pkl', 'rb') as f:
    _ibd_results = pickle.load(f)
_counts = _ibd_results['consensus_counts']
ranked = sorted(_counts.items(), key=lambda kv: -kv[1])
CONSENSUS = [g for g, _ in ranked[:5]]
print("Full ranked consensus_counts (gene, fold_count):")
for _g, _c in ranked:
    print(f"  {_g}: {_c}")
print(f"CONSENSUS (top 5): {CONSENSUS}")
...
# standardize on TAURUS (train) stats, clip at 10 — independently re-implemented from pipeline.py, not a shared code path (outputs verified against the committed artifacts, not by an equivalence test)
