...
Independently re-implemented from filter_and_save_baseline_final.py (QC) and
ml_pipeline.py (LOO/perm); not a shared code path. Outputs were verified to
match the committed artifacts rather than enforced by a shared module or an
equivalence test.
...
# --- QC independently re-implemented from filter_and_save_baseline_final.py; not a shared code path (outputs verified against the committed artifacts, not by an equivalence test) ---
...
# --- permutation (independently re-implemented from ml_pipeline.py; not a shared code path — outputs verified against the committed artifacts, not by an equivalence test) ---
...
ge = int((perm >= acc).sum()); p_emp = float((ge + 1) / (len(perm) + 1))
